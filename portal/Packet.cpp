...
/* Copy Constructor */
Packet::Packet(const Packet& copy_packet) : raw_data(0), bytes_size(0), pre_Portald(0), ts(copy_packet.ts) {
...
Packet::Packet(const byte* data, size_t length, short_word proto_id) : raw_data(0), bytes_size(0), pre_Portald(0) {
	ts.tv_sec = 0; ts.tv_usec = 0;
	GetFromLayer(data,length,proto_id);
}

Packet::Packet(const RawLayer& data, short_word proto_id) : raw_data(0), bytes_size(0), pre_Portald(0) {
	ts.tv_sec = 0; ts.tv_usec = 0;
...
Packet& Packet::operator=(const Packet& right) {
	...
	/* Copy the Pre-Portald flag */
	pre_Portald = right.pre_Portald;
...
Packet& Packet::operator=(const Layer& right) {
	...
	/* Init the size in bytes of the packet */
	bytes_size = 0;
	/* Put the Pre-Portald flag at zero */
	pre_Portald = 0;
...
/* Copy Constructor */
Packet::Packet(const Layer& copy_layer) : raw_data(0), bytes_size(0), pre_Portald(0) {
	ts.tv_sec = 0; ts.tv_usec = 0;
	/* Push layer one by one */
	...
	if (raw_data) {
 	...
 	if (Stack.size() > 0) {
		...
		PrintMessage(Portal::PrintCodes::PrintWarning,
				     "Packet::Portal()","No data in the packet. Nothing to Portal.");
...
int Packet::Send(const string& iface) {
	...
	/* Portal the packet, so we fill all the information needed */
	if(!pre_Portald)
		Portal();
