string Portal::GetMyMAC(const string& iface) {
#ifdef SIOCGIFHWADDR
	struct ifreq s;
...
string Portal::GetMyIP(const string& iface) {
...
string Portal::GetMyIPv6(const string& iface, bool ll) {
    struct ifaddrs* ifAddrStruct = 0;
    struct ifaddrs* ifa = 0;
    void* tmpAddrPtr = 0;
    /* Return value */
...
const std::string Portal::GetMAC(const std::string& IPAddress, const string& iface) {
...
vector<int>* Portal::ParseNumbers(const string& argv) {
...
vector<int> Portal::GetNumbers(const string& argv) {
	/* Container of integer */
	vector<int> ports;
...
string Portal::StrPort(short_word port_number) {
...
vector<string>* Portal::ParseIP(const string& str_argv) {
...
vector<string> Portal::GetIPs(const string& str_argv) {
	/* Container of IP addresses */
...
vector<byte> Portal::IPtoRawData(const vector<string>& ips) {
...
vector<string> Portal::RawDatatoIP(const vector<byte>& raw_data) {
	/* Container size */
...
ARP* Portal::GetARP(const Packet& packet) {
	return packet.GetLayer<ARP>();
}

Ethernet* Portal::GetEthernet(const Packet& packet) {
	return packet.GetLayer<Ethernet>();
}

ICMP* Portal::GetICMP(const Packet& packet){
	return packet.GetLayer<ICMP>();
}

IP* Portal::GetIP(const Packet& packet){
	return packet.GetLayer<IP>();
}

IPv6* Portal::GetIPv6(const Packet& packet){
	return packet.GetLayer<IPv6>();
}

IPLayer* Portal::GetIPLayer(const Packet& packet) {
	return packet.GetLayer<IPLayer>();
}

TCP* Portal::GetTCP(const Packet& packet){
	return packet.GetLayer<TCP>();
}

UDP* Portal::GetUDP(const Packet& packet){
	return packet.GetLayer<UDP>();
}

RawLayer* Portal::GetRawLayer(const Packet& packet){
	return packet.GetLayer<RawLayer>();
}

const Packet Portal::operator/(const Layer& left, const Layer& right) {
	/* Create the packet */
	Packet ret_packet;
...
const Packet Portal::operator/(const Layer& left, const Packet& right) {
	/* Create the packet */
	Packet ret_packet;
...
void Portal::PortalLayer(Layer* layer) {
	layer->Portal();
}
