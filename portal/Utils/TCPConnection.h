...
namespace Portal {
	...
	void PckHand(Portal::Packet* sniff_packet, void* user);

	class TCPBuffer {

		/* A table that relates the sequence numbers and the data on the segment */
		std::map<uint64_t,Portal::Payload> seq_table;
		...
		void ReassembleData(Portal::Payload& buffer);
	...
	public:
		...
		void Add(word seq, const Portal::Payload& payload);
		 ...
		 */
		byte Push(word seq, Portal::Payload& buffer);
	...
	public:
		...
		*/
		...
		friend void* Portal::ConnectHandler(void* thread_arg);
		friend void Portal::PckHand(Portal::Packet* sniff_packet, void* user);
