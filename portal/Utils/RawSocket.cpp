...
int Portal::SocketSender::CreateLinkSocket(word protocol_to_sniff)
{
	int rawsock;
#ifdef __APPLE__
	int i;
...
int Portal::SocketSender::CreateRawSocket(word protocol_to_sniff)
{
...
int Portal::SocketSender::CreateRaw6Socket(word protocol_to_sniff) {
    /* Create a socket descriptor */
...
int Portal::SocketSender::BindLinkSocketToInterface(const char *device, int rawsock, word protocol)
{
	struct ifreq ifr;
#ifdef __APPLE__
	int i;
...
int Portal::SocketSender::BindRawSocketToInterface(const char *device, int s)
{
	/* Bind to interface */
#ifndef __APPLE__
    ifreq Interface;
...
int Portal::SocketSender::SendLinkSocket(int rawsock, byte *pkt, size_t pkt_len)
{
	return write(rawsock, pkt, pkt_len);
}

int Portal::SocketSender::SendRawSocket(int rawsock, struct sockaddr* din, size_t size_dst, byte *pkt, size_t pkt_len)
{
	int ret;
...
int Portal::SocketSender::SendSocket(int rawsock, word proto_id, byte *pkt, size_t pkt_len) {
	if(proto_id == IP::PROTO) {
		/* Raw socket, IPv4 */
...
Portal::SocketSender::~SocketSender() {
