...
void Proto::PrintHdr() const {
	...
	Newline(out);
	Printline(out,"namespace Portal {",0);
	Newline(out);
...
void Proto::PrintConstructorCpp(ostream& out) const {
	...
	Newline(out);
	Printline(out,"using namespace Portal;",0);
...
void Proto::PrintPortalCpp(ostream& out) const {
	/* Copy to put the name uppercase */
	...
	/* Portal function */
	Printline(out,"void " + protoName + "::Portal() {",0);
...
void Proto::PrintCpp() const {
	...
	if(fexists(protoName+"Portal.cpp"))
		return;

	string out_Portal(protoName+"Portal"+".cpp");

	std::ofstream file_Portal(out_Portal.c_str());

	/* Print license */
	if(PrintLicense(file_Portal,"../../LICENSE"))
		if(PrintLicense(file_Portal,"ProtoSource/LICENSE"))
			if(PrintLicense(file_Portal,"../LICENSE"))
				cerr << "[@] Unable to open the license file when writing header. " << endl;
	Newline(file_Portal);

	PrintPortalCpp(file_Portal);
