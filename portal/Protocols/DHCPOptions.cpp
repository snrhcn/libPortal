...
DHCPOptions* Portal::CreateDHCPOption(short_word code, const string& str) {
	/* Just a generic name... */
	return new DHCPOptionsString(code,str);
}

DHCPOptions* Portal::CreateDHCPOption(short_word code,const vector<string>& ip_addresses) {
	return new DHCPOptionsIP(code,ip_addresses);
}

/* This get the value of the DHCP options data from a word, short or a byte */
DHCPOptions* Portal::CreateDHCPOption(short_word code, word value, byte type_tag) {
...
DHCPOptions* Portal::CreateDHCPOption(short_word code, const byte* raw_data, size_t length) {
	if(code == DHCPOptions::DHCPMsgType) {
		if(length >= 1)
