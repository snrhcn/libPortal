...
void Portal::Verbose(byte value) {
	Portal::ShowWarnings = value;
}

void Portal::Layer::Bind(const Layer& bottom_layer, short_word proto_id) {
...
short_word Portal::Layer::CheckBinding() const {
...
short_word Portal::CheckSum(short_word *buf, int nwords) {
	unsigned long sum;
...
void Portal::Layer::HexDump(ostream& str) const {
...
/* Print Payload */
void Portal::Layer::RawString(ostream& str) const {
...
void Portal::Layer::PrintFields(std::ostream& str) const {
	/* Print the fields */
	Fields.Print(str);
}

void Portal::Layer::PrintPayload(std::ostream& str) const {
	str << "Payload = ";
	LayerPayload.Print(str);
}

void Portal::Layer::Print(std::ostream& str) const {
	str << "< ";
...
void Portal::Layer::allocate_words(size_t nwords) {
	allocate_bytes(nwords * sizeof(word));
}

/* Allocate a number of bytes into the layer */
void Portal::Layer::allocate_bytes(size_t nbytes) {
...
size_t Portal::Layer::GetData(byte* data) const {
	/* Copy the data */
	if (raw_data)
...
size_t Portal::Layer::GetRawData(byte* data) const {
	/* Copy the data */
	if (raw_data) {
...
size_t Portal::Layer::PutData(const byte* data) {
...
void Portal::Layer::ParseData(ParseInfo* info) {
...
void Portal::Layer::ParseLayerData(ParseInfo* info) {
	/* No more layers, default */
	info->top = 1;
}

void Portal::Layer::RedefineField(size_t nfield) {
	/* Set field as active */
	Fields.SetActive(nfield);
}

size_t Portal::Layer::GetRemainingSize() const {
	if (!TopLayer)
		return GetSize();
	else
...
/* Set payload */
void Portal::Layer::SetPayload (const byte *data, int ndata) {
	LayerPayload.SetPayload(data,ndata);
	bytes_size = size + LayerPayload.GetSize();
}

/* Add more stuff to the payload */
void Portal::Layer::AddPayload (const byte* data, int ndata) {
	LayerPayload.AddPayload(data,ndata);
	bytes_size = size + LayerPayload.GetSize();
}

/* Set payload */
void Portal::Layer::SetPayload (const char *data) {
	LayerPayload.SetPayload(data);
	bytes_size = size  + LayerPayload.GetSize();
}

/* Add more stuff to the payload */
void Portal::Layer::AddPayload (const char* data) {
	LayerPayload.AddPayload(data);
	bytes_size = size + LayerPayload.GetSize();
}

/* Set payload */
void Portal::Layer::SetPayload (const Payload& data)  {
	LayerPayload.SetPayload(data);
	bytes_size = size + LayerPayload.GetSize();
}

/* Add more stuff to the payload */
void Portal::Layer::AddPayload (const Payload& data) {
	LayerPayload.AddPayload(data);
	bytes_size = size + LayerPayload.GetSize();
}

/* Copy the data into the pointer and returns the number of bytes copied */
size_t Portal::Layer::GetPayload(byte* dst) const {
	return LayerPayload.GetPayload(dst);
}

Portal::Layer::Layer() : size(0), bytes_size(0), raw_data(0), BottomLayer(0), TopLayer(0) { }

Portal::Layer::Layer(const Layer& layer) : raw_data(0) {
	/* Put size to zero */
	size = 0;
...
Layer& Portal::Layer::operator=(const Layer& right) {
...
void Portal::Layer::Clone(const Layer& layer) {
	/* Delete memory allocated */
	delete [] raw_data;
	raw_data = 0;
...
FieldInfo* Portal::Layer::GetFieldPtr(size_t nfield) {
	return Fields[nfield];
}

byte Portal::Layer::IsFieldSet(size_t nfield) const {
	return Fields[nfield]->IsFieldSet();
}

void Portal::Layer::ResetFields() {
	Fields.Apply(&FieldInfo::ResetField);
}

void Portal::Layer::ResetField(size_t nfield) {
	Fields[nfield]->ResetField();
}

byte Portal::RNG8() {return rand()%256; }
short_word Portal::RNG16() {return rand()%65536; }
word Portal::RNG32() {return 2 * rand(); }
