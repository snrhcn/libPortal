...
namespace Portal {
    ...
    protected:
		...
		friend void PortalLayer(Layer* layer);
	...
	class Protocol {
		...
		typedef Portal::Layer(*(*Constructor)());
	...
	public:
		...
		void Register(Portal::Layer* ProtoLayer) {
			/* Get this Layer Name */
		...
		Portal::Layer* GetLayerByName(const std::string& ProtoName) {
			if (RegProtoByName.find(ProtoName) != RegProtoByName.end())
		...
		Portal::Layer* GetLayerByID(const short_word ProtoID) {
...
template<class T>
void Portal::Layer::SetFieldValue(size_t nfield, T HumanValue){
...
template<class T>
T Portal::Layer::GetFieldValue(size_t nfield) const {
